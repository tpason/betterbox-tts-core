...
class VoiceEncoder(nn.Module):
    ...
    def inference(self, mels: torch.Tensor, mel_lens, overlap=0.5, rate: float=None, min_coverage=0.8, batch_size=None):
        ...
        if len_diff > 0:
            # Giữ cùng dtype/device với input để tránh cast thừa khi infer nhiều lần.
            pad = torch.zeros(
                (mels.size(0), len_diff, self.hp.num_mels),
                dtype=mels.dtype,
                device=mels.device,
            )
    ...
    def embeds_from_mels(
        ...
        # Mới — đảm bảo detach trước khi numpy:
        with torch.inference_mode():
            utt_embeds = self.inference(mels.to(self.device), mel_lens, batch_size=batch_size, **kwargs).detach().cpu().numpy()
