...
def test_seed_glossary_and_terms() -> None:
    print("\n[seed glossary + term mining]")
    from extract_term_glossary import mine_candidate_terms
    from story_memory import (
        StoryMemory,
        _relevant_seed_glossary,
        apply_seed_glossary_replacements,
        load_seed_glossary,
    )

    mem = StoryMemory(replacements={"Luyện Khí Tầng 1": "Luyện Khí Kỳ Một"})
    apply_seed_glossary_replacements(mem, "tien_hiep")
    check("story replacement thắng seed", mem.replacements["Luyện Khí Tầng 1"] == "Luyện Khí Kỳ Một")
    check("seed replacement được thêm", "Tai Họa Từ Trời" in mem.replacements)

    seed = load_seed_glossary("tien_hiep")
    story_glossary = [{"source_terms": ["Qi Refining"], "canonical_vi": "Luyện Khí Kỳ"}]
    relevant = _relevant_seed_glossary(seed, story_glossary, "Qi Refining tầng một", max_items=20)
    check("story source term loại seed conflict", all("Qi Refining" not in (item.get("source_terms") or []) for item in relevant))

    candidates = mine_candidate_terms([
        "He activated 《Nine Yang True Fire Seal》 once and the cave shook."
    ], min_count=3)
...
def test_untranslated_slang_warning() -> None:
    print("\n[untranslated slang]")
    from check_translation_quality import detect_unlocalised_slang, run_full_quality_check

    text = "Đây là một đoạn tiếng Việt đủ dài để kiểm tra chất lượng. Nhân vật này quá OP và cần buff ngay lập tức. " * 2
    found = detect_unlocalised_slang(text, genre="western_fantasy")
    check("detect OP/buff", {"OP", "buff"}.issubset(set(found)))
    blocking, warnings = run_full_quality_check(text, genre="western_fantasy")
    check("slang không blocking", not any(i.startswith("untranslated_slang") for i in blocking))
    check("slang là warning", any(i.startswith("untranslated_slang") for i in warnings))

    system_text = "Đây là đoạn tiếng Việt có bảng trạng thái với HP, MP và EXP hợp lệ trong truyện hệ thống. " * 2
    check("he_thong cho phép HP/MP/EXP", detect_unlocalised_slang(system_text, genre="he_thong") == [])
...
def main() -> int:
    ...
    test_seed_glossary_and_terms()
    test_untranslated_slang_warning()
